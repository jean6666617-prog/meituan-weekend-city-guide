...
const activities = [
  {name:"西岸艺术慢逛计划",type:"看展",time:"周六 10:30",place:"西岸美术馆",price:60,match:97,pressure:"低压力",solo:true,people:"1—2 人",feature:"不设固定路线，戴上耳机慢慢看完整场展。",img:"assets/hero-exhibition.jpg",modes:["solo","soft"],weather:["晴天","雨天"]},
  {name:"梧桐区日落散步",type:"户外",time:"周六 16:40",place:"武康路集合",price:0,match:95,pressure:"轻互动",solo:true,people:"1—4 人",feature:"沿着光线走，不赶景点；想聊天就聊，不聊也自在。",img:"assets/hero-hiking.jpg",modes:["solo","soft"],weather:["晴天"]},
  {name:"城市天台日落音乐会",type:"演出",time:"周六 18:00",place:"静安大悦城",price:128,match:94,pressure:"自由社交",solo:true,people:"1—6 人",feature:"设有独处聆听区和搭子互动区，社交电量自由选择。",img:"assets/hero-concert.jpg",modes:["solo","soft","party"],weather:["晴天"]},
  {name:"上生新所黑胶市集",type:"市集",time:"周日 13:00",place:"上生·新所",price:50,match:92,pressure:"自然交流",solo:true,people:"1—4 人",feature:"黑胶、咖啡与旧物摊位，不用破冰也能自然聊起来。",img:"assets/hero-exhibition.jpg",modes:["solo","soft","party"],weather:["晴天","雨天"]},
  {name:"苏州河夜骑",type:"城市夜游",time:"周六 19:30",place:"昌平路桥",price:35,match:90,pressure:"团队协作",solo:false,people:"3—8 人",feature:"领队控速，全程 12km；沿河夜景和三个拍照停靠点。",img:"assets/hero-hiking.jpg",modes:["soft","party"],weather:["晴天"]},
  {name:"小型陶艺体验局",type:"手作",time:"周日 14:00",place:"愚园路",price:168,match:91,pressure:"低压力",solo:true,people:"1—4 人",feature:"四人小班，各做各的也不尴尬，完成后可自由离开。",img:"assets/hero-exhibition.jpg",modes:["solo","soft"],weather:["晴天","雨天"]},
  {name:"云南街松弛吃逛",type:"美食",time:"周日 11:30",place:"云南南路",price:88,match:89,pressure:"轻互动",solo:true,people:"1—3 人",feature:"三家小店组成半日路线，吃多少、走多快都随你。",img:"assets/hero-concert.jpg",modes:["solo","soft"],weather:["晴天","雨天"]},
  {name:"夜幕桌游新朋友局",type:"随便看看",time:"周六 19:00",place:"大学路",price:78,match:93,pressure:"高互动",solo:false,people:"5—8 人",feature:"主持人带场，第一次见面也不用担心冷场。",img:"assets/hero-concert.jpg",modes:["party"],weather:["晴天","雨天"]}
];
...
function renderAnimals(){
  $("#animalSelector").innerHTML=animals.map(animal=>`<button class="animal-option ${animal.id===state.animal?"selected":""}" data-animal="${animal.id}" type="button" role="option" aria-selected="${animal.id===state.animal}" aria-label="${animal.name}：${animal.trait}" title="${animal.name} · ${animal.trait}"><img src="${animal.img}" alt="" /></button>`).join("");
  const animal=getAnimal(state.animal);
  $("#mainAnimal").src=animal.img;$("#mainAnimal").alt=`${animal.name}城市伙伴`;
  $("#headerAvatar").src=animal.img;$("#headerAvatar").alt=`${animal.name}头像`;$("#profileButton").setAttribute("aria-label",`当前动物伙伴：${animal.name}`);
  $("#passportAvatar").src=animal.img;$("#passportAvatar").alt=`护照中的${animal.name}头像`;
...
function closeHotModal(){modal.classList.remove("open");modal.setAttribute("aria-hidden","true");document.body.style.overflow="";sessionStorage.setItem("weekend-hot-seen","1");previousFocus?.focus()}
...
if(!sessionStorage.getItem("weekend-hot-seen"))setTimeout(openHotModal,450);
